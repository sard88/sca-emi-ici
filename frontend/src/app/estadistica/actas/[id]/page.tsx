...
export default function EstadisticaActaDetallePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setError(null);
      try {
        setData(await getEstadisticaActaDetalle(params.id));
      } catch (err) {
    ...
    <AppShell showRightPanel={false}>
      {!user ? null : !canAccessEstadisticaActas(user) ? (
        ...
        <div className="space-y-6">
          <PageHeader title="Detalle operativo de acta" description="Vista solo lectura para seguimiento y exportación autorizada." user={user} />
          <Link className="inline-flex rounded-xl border border-[#d8c5a7] px-4 py-2 text-sm font-black text-[#6f4a16]" href="/estadistica/actas">
            Volver a consulta
          </Link>
