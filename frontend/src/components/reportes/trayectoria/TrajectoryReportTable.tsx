import type { ReporteTrayectoriaColumna, ReporteTrayectoriaItem } from "@/lib/types";

const MAX_PREVIEW_ROWS = 100;

export function TrajectoryReportTable({ columns, items }: { columns: ReporteTrayectoriaColumna[]; items: ReporteTrayectoriaItem[] }) {
  const safeColumns = columns.filter((column) => !column.key.toLowerCase().includes("matricula"));
