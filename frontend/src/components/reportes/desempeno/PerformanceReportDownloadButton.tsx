...
import type { DownloadResult, ReporteDesempenoCodigo } from "@/lib/types";
import { descargarReporteDesempenoXlsx } from "@/lib/api";

export function PerformanceReportDownloadButton({
  slug,
  filters,
  onDone,
  onError,
  ...
  async function handleDownload() {
    setLoading(true);
    try {
      const result = await descargarReporteDesempenoXlsx(slug, filters);
      onDone(result);
    } catch (err) {
